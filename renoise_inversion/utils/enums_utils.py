...
from renoise_inversion.schedulers.euler_scheduler import (
    MyEulerAncestralDiscreteScheduler,
)
from renoise_inversion.schedulers.lcm_scheduler import MyLCMScheduler
from renoise_inversion.schedulers.ddim_scheduler import MyDDIMScheduler
from renoise_inversion.pipes.sdxl_inversion_pipeline import SDXLDDIMPipeline
from renoise_inversion.pipes.sd_inversion_pipeline import SDDDIMPipeline

from attend_and_excite.pipeline_stable_diffusion_xl_img2img_attend_and_excite import (
    StableDiffusionXLImg2ImgAttendAndExcitePipeline,
)
...
def model_type_to_class(model_type, is_aae=False):
    if model_type == Model_Type.SDXL:
        if is_aae:
            return StableDiffusionXLImg2ImgAttendAndExcitePipeline, SDXLDDIMPipeline
...
def _get_pipes(model_type, device, is_aae=False):
    model_name = model_type_to_model_name(model_type)
    pipeline_inf, pipeline_inv = model_type_to_class(model_type, is_aae=is_aae)
...
def get_pipes(model_type, scheduler_type, device="cuda", is_aae=False):
    scheduler_class = scheduler_type_to_class(scheduler_type)

    pipe_inversion, pipe_inference = _get_pipes(model_type, device, is_aae=is_aae)
