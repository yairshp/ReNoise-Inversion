...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        diffusers.     "stabilityai/stable-diffusion-xl-refiner-1.0", torch_dtype=torch.float16
        diffusers. )
        >>> pipe = pipe.to("cuda")
...
class StableDiffusionXLImg2ImgMaskEditPipeline(
    DiffusionPipeline,
    TextualInversionLoaderMixin,
    FromSingleFileMixin,
    StableDiffusionXLLoraLoaderMixin,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(image_latents, list) and any(
            isinstance(l, torch.Tensor) and l.ndim == 5 for l in image_latents
        ):
        ...
        if image_latents.ndim == 4:
            image_latents = image_latents.reshape(
                batch_size, len(timesteps), *latent_shape
            )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = latents * (1 - mask_image) + image_latents[i] * mask_image
