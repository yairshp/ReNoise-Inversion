...
def find_indices(prompt, objects, model):
    ...
    prompt_tokens = [
        model.tokenizer.convert_ids_to_tokens(x)
        for x in model.tokenizer.encode(prompt_split)
    ]
    ...
    for token in objects:
        if token not in prompt_tokens:
            ...
            continue
        token_index = [
            i for i, prompt_token in enumerate(prompt_tokens) if prompt_token == token
        ]
    ...
    if len(tokens_indices) == 0:
        ...
        for object in objects:
            ...
            start_idx = find_sub_np_array_in_larger_array(
                large_array=prompt_encoded, small_array=object_tokens_ids
            )
    ...
    print(prompt)
    print(objects)
    # for t in tokens_indices:
    #     print(prompt_tokens[t[0]])
