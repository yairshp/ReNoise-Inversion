...
def Transformer2DModelForward(
    ...
    if self.is_input_continuous:
        ...
        if not self.use_linear_projection:
            ...
            hidden_states = hidden_states.permute(0, 2, 3, 1).reshape(
                batch, height * width, inner_dim
            )  # hidden_states.shape == (2,320,96,96) -> (2,9216,320)
            hidden_states = self.proj_in(hidden_states)
...
def attn_call(
    ...
    if hasattr(self, "store_attn_map"):
        ...
        self.attn_map = rearrange(
            attention_probs, "b (h w) d -> b d h w", h=height
        )  # (10,9216,77) -> (10,77,96,96)
    ####################################################################################################
...
def lora_attn_call(
    self, attn: Attention, hidden_states, height, width, *args, **kwargs
):
