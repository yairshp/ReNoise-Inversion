...
# from attention_maps_utils import get_attn_maps
...
class StableDiffusionXLImg2ImgAttendAndExcitePipeline(
    DiffusionPipeline,
    TextualInversionLoaderMixin,
    FromSingleFileMixin,
    StableDiffusionXLLoraLoaderMixin,
    ...
    def __call__(
        ...
        # thresholds: dict = {0: 0.03, 10: 0.3, 20: 0.5},  # NEW
        ...
        **kwargs,
        ...
        # height = image.height or image.shape[-2]
        # width = image.width or image.shape[-1]
            ...
            for i, t in enumerate(timesteps):
                ...
                if not disable_aae:
                    ...
                    latents = self.aae_step(
                        ...
                        add_text_embeds_aae,
                        add_time_ids_aae,
                        to_save_images=True if "hammock" in prompt else False,
                    )
        ...
        attention_maps = (
            self.attention_store.aggregate_attention(
                from_where=("up", "down", "mid"),
            )[:, :, 1:-1]
            .detach()
            .cpu()
        )
        attention_map = attention_maps[:, :, token_indices[0][0]] * 255
        attention_map = torchvision.transforms.functional.to_pil_image(
            attention_map
        ).resize((width, height))
        ...
        return StableDiffusionXLPipelineOutput(images=image), attention_map
    ...
    def _compute_max_attention_per_index(
        ...
        # attention_for_text = 2 * (attention_for_text - attention_for_text.min()) / (attention_for_text.max() - attention_for_text.max()) - 1
        ...
        for i in indices:
            ...
            ## DEBUG
            # if to_save_images:
            #     # attn_map_image = to_pil_image(image).resize((512, 512))
            # attn_map_image.save("attn_maps/attn_map.png")
...
if __name__ == "__main__":
    ...
    init_image = (
        load_image("example_images/mini_benchmark/A stop sign in a dirt road.png")
        .convert("RGB")
        .resize((1024, 1024))
    )
    # init_image = load_image(url).convert("RGB")
    # prompt = "a photo of an man riding a horse on mars"
    prompt = "A stop sign in a dirt road with a car in the background"
    tokens = ["car"]

    model = StableDiffusionXLImg2ImgAttendAndExcitePipeline.from_pretrained(
        "stabilityai/stable-diffusion-xl-base-1.0",
